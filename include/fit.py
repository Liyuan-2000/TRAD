...
def fit(net,
    ...
    if optim == 'no_reg':
        ...
        for i in range(num_iter):
            ...
            W = W + rho * (v - F.pad(x, (0, m - d, 0, m - d), "constant", 0))
            # calculate loss
            output = apply_f(x, m)
        ...
        for i in range(num_iter):
            ...
            temp1 = torch.sqrt(b ** 2 + ksi * I) / torch.sqrt(temp3 ** 2 + ksi * I)
            v = ifftn(temp1 * temp2)
            x = F.pad(v + W / rho, (0, d - m, 0, d - m ), "constant", 0) - alpha_ / rho * tv_grad2(F.pad(v + W / rho, (0, d - m, 0, d - m ), "constant", 0)).to(device)
            W = W + rho * (v - F.pad(x, (0, m - d, 0, m - d), "constant", 0))
            # calculate loss
            output = apply_f(x, m)
            loss_LS = mse(output, img_clean_var)
